...
@file: fanPassActivity.py
@time: 2019/7/16 19:02
@desc: 粉丝通活动
"""
import json
import random
...
class FanPass(Public):
    def __init__(self, account, activityId):
    ...
    def get_formData(self):
        print("正在获取表单")
        url = 'https://cpub.alibaba.com/render.json'
        params = {
            'activityId': self.activityId,
            'redirectURL': '//creator.alibaba.com/submission/activity/manage',
            'template': 'icbuVideo',
        }
        response = requests.get(url, headers=self.headers, params=params, verify=False)
        formData = response.json()['config']['formData']
        return formData
    ...
    def save(self, formData, actions, children):
        ...
        # url = "https://cpub.alibaba.com/submit.json?draft=1&_draft_id=1&_tb_token_=" + self.tb_token
        data = {
            'config': json.dumps({
                "actions": actions,
                "children": children,
        ...
        if len(response.text) > 200:
            ...
            data['children'][-1].pop('errMsg')
            data['children'][-1]['props']['value']['hasRead'] = True
            data['children'][-1]['props']['value']['hidden'] = True
            data = {
...
class FanPassActivity(Public):
    def __init__(self, account, skus):
        self.skus = skus
        self.account = account
        super(FanPassActivity, self).__init__(self.account)

    def main(self, activityUrl):
        try:
            activityId = activityUrl.split('/')[-1]
            canBePublishedCountsUrl = 'https://contents.alibaba.com/api2/activity/activity_info.json?id=' + activityId
            canBePublishedCountsResponse = requests.get(canBePublishedCountsUrl, headers=self.headers, verify=False)
            canBePublishedCounts = canBePublishedCountsResponse.json()['data']['remaindeToDayActivityStr'][:-1]
            publishedCountsUrl = 'http://cs1.jakcom.it/alibaba/query_feed_promotion?account=' + self.account
            publishedCountsResponse = requests.get(publishedCountsUrl)
            publishedCounts = publishedCountsResponse.text
            if int(canBePublishedCounts) == 0:
                return 'The number of posts has reached the limit'
            print(canBePublishedCounts)
            skuList = self.skus.split(',')[:int(canBePublishedCounts)][:(8 - int(publishedCounts))]
            for sku in skuList:
                sku = sku.strip()
                print(sku)
                fanPass = FanPass(self.account, activityId)
                fanPass.main(sku)
            return 'true'
        except Exception as e:
            return str(e)
...
def main():
    activityUrl = 'https://creator.alibaba.com/submission/activity/8670'
    account = 'tx@jakcom.com'
    skus = 'R3'
    fanPassActivity = FanPassActivity(account, skus)
    msg = fanPassActivity.main(activityUrl)
    return msg
