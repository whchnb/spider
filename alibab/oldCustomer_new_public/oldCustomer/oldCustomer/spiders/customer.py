...
class CustomerSpider(scrapy.Spider):
        ...
        if numbers != 0:
            ...
            for index, data in enumerate(datas):
                try:
                    ...
                    originSaleName = data['originSaleName'] if data['originSaleName'] is not None else '公海客户无原业务员'  # 原业务员
                    potentialScore = data.get('potentialScore', '公海客户无潜力分')  # 潜力分
                    ...
                    willLoss = willLoss_dict[data.get('willLoss', '公海客户无流失预警')]  # 流失预警
                    isDing = data.get('isDing', '公海客户无钉住状态')  # 钉住状态
                    # 用户数据
                    costomer_data = {
                        'saleName': saleName,
    ...
    def send_name_card(self, response):
        public = response.meta['public']
        contactId = response.meta['contactId']
        customerId = response.meta['customerId']
